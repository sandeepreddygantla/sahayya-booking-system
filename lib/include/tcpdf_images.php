...
// - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
	...
	public static function _parsejpeg($file) {
		...
		$a = self::booklyGetImageSize($file);
		if (empty($a)) {
		...
		// get file content
		$data = self::booklyFileGetContents($file);
     ...
     * @return array|false
     */
    public static function booklyGetImageSize( $file )
    {
     ...
     */
    public static function booklyFopen( $file )
    {
        $fp = @fopen( $file, 'rb' );
        if ( $fp === false ) {
            $headers = array(
            ...
            $fp = @fopen( $file, 'rb', false, $context );
        }
     ...
     * @return false|string
     */
    public static function booklyFileGetContents( $file )
    {
        $fp = self::booklyFopen( $file );
        $source = '';
        while ( ! feof( $fp ) ) {
